...
# for training data
features, norm_features, labels = dm.training_data_preprocessing(cut_sec=CUT_SEC,
                                                                 n_mfcc=N_MFCC,
                                                                 sr=SR,
                                                                 splited=SPLITED_CNT)

print(np.shape(features), np.shape(norm_features), np.shape(labels))
np.savez_compressed(file=f"D:/AI/npz/english-accent-classification/splited_training_dataset_cut-sec-{CUT_SEC}_n-mfcc-{N_MFCC}_sr-{SR}_2500.npz",
